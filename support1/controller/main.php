        ...
        if (isset($methods[$path[2]])) {
          ...
          foreach ($details['params'] as $param) {
            ...
            try{
              if ($var){
                try{
                  if(isset($param['pattern'])){//if there is a pattern 
                    if(preg_match($patterns[$param['pattern']]['regex'],$var)){//if the var matches the pattern
                ...
                try{
                  if(isset($param['default'])){
                    $request[$param['name']]=$param['default'];// if there is a default value
                  }
